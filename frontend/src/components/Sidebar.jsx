...
export default function Sidebar({ onLogout }) {
  return (
    <aside className="flex w-64 flex-shrink-0 flex-col bg-brand-deep px-5 py-6 text-white">
      <div className="px-2">
        <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-sage-300/80">
          OdontoGPT
        </p>
      ...
      <nav className="flex flex-1 flex-col gap-1" aria-label="Navegação principal">
        {NAV_ITEMS.map(({ to, end, label, icon: Icon }) => (
          ...
          >
            {({ isActive }) => (
              <>
                <Icon size={18} strokeWidth={1.9} className={isActive ? "text-sage-300" : "text-white/55 group-hover:text-white/80"} />
